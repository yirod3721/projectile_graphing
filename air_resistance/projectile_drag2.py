...
# Input validation for mass and initial velocity
while True:
    try:
        m = float(input("Enter the mass of the projectile (kg): "))
        if m <= 0:
            print("Mass must be positive. Please try again.")
            continue
        break
    except ValueError:
...
while True:
    try:
        ...
        if u <= 0:
            print("Initial velocity must be positive. Please try again.")
            continue
        break
    except ValueError:
        print("Invalid input. Please enter a positive number for initial velocity.")
...
# Initialize rho to sea level value
rho = rho_sea_level  # Initial air density (sea level)
...
while t <= t_max and not stop_simulation:
    ...
    # Adjust air density gradually with altitude
    if y >= 12000:
        rho = 0.18 * rho_sea_level  # Reduced air density at high altitude
    else:
        # Linear interpolation model for air density change with altitude
        rho = rho_sea_level * (1 - (y / 100000))
